      ...
      <svg viewBox={`0 0 ${width} ${height}`} className={cx("h-full w-full", className)} preserveAspectRatio="none">
        ...
        <defs>
          ...
          <linearGradient id={gradId} x1="0" x2="0" y1="0" y2="1">
            {" "}
            <stop offset="0%" stopColor={colors.gradFrom} stopOpacity="0.6" /> <stop offset="100%" stopColor={colors.gradTo} stopOpacity="0" />{" "}
          </linearGradient>{" "}
        </defs>{" "}
        <path d={areaPath} fill={`url(#${gradId})`} /> <path d={linePath} fill="none" stroke={colors.stroke} strokeWidth="1.5" strokeLinecap="round" />{" "}
      </svg>
    );
  }
