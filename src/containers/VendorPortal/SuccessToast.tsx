...
function SuccessToast({ isVisible, onDismiss, onViewOrder }: { isVisible: boolean; onDismiss: () => void; onViewOrder?: () => void }) {
  useEffect(() => {
    if (isVisible) {
        ...
        <div className="flex gap-4 pr-8">
          ...
          <div className="flex flex-1 flex-col gap-3 pt-0.5">
            ...
            <div className="flex flex-col gap-1">
              {" "}
              <p className="text-sm font-semibold text-[#181d27]"> Incoming order accepted </p>{" "}
              <p className="text-sm text-[#414651]"> You have accepted a new order. </p>{" "}
            </div>{" "}
            {}{" "}
            <div className="flex gap-3">
              ...
              {onViewOrder && (
                <button type="button" onClick={onViewOrder} className="text-sm font-semibold text-[#0948b5] hover:text-[#073d8a]">
                  {" "}
                  View order{" "}
                </button>
              )}{" "}
