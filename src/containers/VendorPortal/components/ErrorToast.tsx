...
function ErrorToast({ isVisible, onDismiss }: { isVisible: boolean; onDismiss: () => void }) {
  useEffect(() => {
    if (isVisible) {
        ...
        <div className="flex gap-4 pr-8">
          ...
          <div className="flex flex-1 flex-col gap-3 pt-0.5">
            ...
            <div className="flex flex-col gap-1">
              {" "}
              <p className="text-sm font-semibold text-[#181d27]"> Incoming order declined </p>{" "}
              <p className="text-sm text-[#414651]"> You declined an incoming order. </p>{" "}
            </div>{" "}
            {}{" "}
