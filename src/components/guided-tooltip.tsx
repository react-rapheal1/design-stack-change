...
export function GuidedTooltip({
  ...
  return (
    <div className={cx("relative w-[380px] rounded-2xl bg-white shadow-xl ring-1 ring-black/[0.06]", className)}>
      {/* Blue dot indicator */}
      <div className="absolute -top-2 -right-2 h-5 w-5 rounded-full bg-brand-600 ring-2 ring-white" />
      ...
      <div className="flex items-center justify-between border-t border-[#eaecf0] px-5 py-3">
        ...
        <div className="flex items-center gap-2">
          {onSkip ? (
            <Button size="sm" color="secondary" onClick={onSkip}>
              Skip
            </Button>
          ) : null}
          <Button size="sm" color="tertiary" iconLeading={isLast ? undefined : ArrowLeft} onClick={onNext}>
            {buttonLabel}
          </Button>
        </div>
      </div>
...
export function ElementHighlight({ elementId: _elementId }: { elementId: string }) {
