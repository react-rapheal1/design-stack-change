...
interface DateRangePickerProps extends AriaDateRangePickerProps<DateValue> {
    ...
    onCancel?: () => void;
    /** Whether to show the preset date range options. Defaults to true. */
    showPresets?: boolean;
}

export const DateRangePicker = ({ value: valueProp, defaultValue, onChange, onApply, onCancel, showPresets = true, minValue, maxValue, isDateUnavailable, ...props }: DateRangePickerProps) => {
    ...
    // Use internal state and sync with external value for proper controlled behavior
    const [internalValue, setInternalValue] = useState<{ start: DateValue; end: DateValue } | null>(valueProp ?? defaultValue ?? null);
    const [focusedValue, setFocusedValue] = useState<DateValue | null>(null);

    // Sync internal value with external prop changes
    const value = valueProp !== undefined ? valueProp : internalValue;

    const setValue = (newValue: { start: DateValue; end: DateValue } | null) => {
    ...
    return (
        <AriaDateRangePicker aria-label="Date range picker" shouldCloseOnSelect={false} {...props} minValue={minValue} maxValue={maxValue} isDateUnavailable={isDateUnavailable} value={value} onChange={setValue}>
            <AriaGroup>
